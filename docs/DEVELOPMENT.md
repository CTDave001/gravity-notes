...
`src-tauri/gen/apple/` is generated by Tauri and remains uncommitted. `npm run ios:init` generates that project and then copies the committed privacy manifest into it. Run `npm run ios:prepare` again whenever the Apple project is regenerated. The cloud workflow also generates `src-tauri/tauri.ios.ci.conf.json` from protected GitHub environment configuration; that file is ignored and must not be committed.
